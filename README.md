...
6. **Замена middleware на proxy** — текущий Next.js Middleware работает на Edge Runtime и проверяет только наличие куки,
   не валидируя токен. Полноценная защита роутов возможна через Route Handlers (`app/api/`) или внешний proxy (nginx,
   Caddy), который верифицирует JWT перед тем как пустить запрос к приложению.
